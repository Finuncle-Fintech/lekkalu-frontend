...
describe('changeExpenseRequest', () => {
  test('successfully deletes an expense', async () => {
    ...
    const amountField =
      screen.getByTestId('amount-expense').firstChild.firstChild;
    const endAdornment =
      screen.getByTestId('tags-expense').children[1].children[3].children[1];

    fireEvent.change(amountField, { target: { value: '23.00' } });
    fireEvent.click(endAdornment);

    const firstOption =
      screen.getAllByRole('presentation')[2].firstChild.firstChild.firstChild;
