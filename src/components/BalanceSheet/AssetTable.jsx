...
export default function EnhancedTable(props) {
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - Object.keys(props.assetDatas).length) : 0

  const visibleRows = React.useMemo(() => {
    if (props.assetDatas && Object.keys(props.assetDatas).length > 0) {
      const sortedData = stableSort(Object.values(props.assetDatas), getComparator(order, orderBy))
          ...
          <Paper sx={{ width: '100%', mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <Box sx={{ display: 'flex', marginRight: '2vw' }}>
                ...
                >
                  Page {page + 1} of {Math.ceil(Object.keys(props.assetDatas).length / rowsPerPage)}
                </Typography>
              ...
              <Table sx={{ minWidth: 750, border: 'none' }}>
                ...
                <TableBody>
                  {visibleRows.map((row, index) => {
                    const isItemSelected = isSelected(props.assetDatas[index].id)
                    ...
                    return (
                      <TableRow
                        hover
                        onClick={(event) => handleClick(event, props.assetDatas[index].id)}
                        role='checkbox'
