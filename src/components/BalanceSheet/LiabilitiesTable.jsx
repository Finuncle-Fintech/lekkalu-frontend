      ...
      Swal.fire({
        ...
        if (res.isConfirmed) {
          try {
            ...
            Swal.fire('Failure', 'Something went wrong while deleting liability!.', 'error')
          } finally {
...
export default function EnhancedTable(props) {
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - Object.keys(props.liabilityDatas).length) : 0

  const visibleRows = React.useMemo(() => {
    if (props.liabilityDatas && Object.keys(props.liabilityDatas).length > 0) {
      const sortedData = stableSort(Object.values(props.liabilityDatas), getComparator(order, orderBy))
  ...
  return (
    <>
      ...
      {showForm && (
        <LiabilitiesForm handleRequestForm={handleRequestForm} showForm={showForm} setForm={setForm} title='Add' />
      )}
          ...
          <Paper sx={{ width: '100%', mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <Box sx={{ display: 'flex', marginRight: '2vw' }}>
                ...
                >
                  Page {page + 1} of {Math.ceil(Object.keys(props.liabilityDatas).length / rowsPerPage)}
                </Typography>
              ...
              <Table sx={{ minWidth: 750, border: 'none' }}>
                ...
                <TableBody>
                  {visibleRows.map((row, index) => {
                    const isItemSelected = isSelected(props.liabilityDatas[index].id)
                    ...
                    return (
                      <TableRow
                        hover
                        onClick={(event) => handleClick(event, props.liabilityDatas[index].id)}
                        role='checkbox'
