...
export default function Summary({ summary }) {
  ...
  return (
    <>
      {summary.length !== 0 && (
        <article className="border rounded-3 d-flex align-items-center justify-content-center p-1 py-md-4 animate__animated animate__fadeIn">
          <div
            style={{
              display: "flex",
            ...
            <Pie dataKey="value" data={data} outerRadius={80} labelLine={false}>
              {data.map((entry, index) => (
                <Cell
                  key={index}
                  fill={
                    entry.name.includes("Total Invested")
                      ? "#099fea"
                      : "#09ea49"
                  }
                />
