...
export default function AssetsdepreciationChart({ data }) {
  ...
  if (!data || data.length === 0 || data[0] === undefined) {
    return <h4>No data for depreciation chart</h4>;
  }
  ...
  let dataToShow = calculateDeprecationData(
    purchase_value,
    depreciation_frequency,
    depreciation_percent,
    sell_date,
  ...
  return (
    <div
      className="section-outer-wrapper col-md-8  bg-red-400"
      ...
      <ResponsiveContainer width="100%" aspect={2}>
        ...
        >
          ...
          <XAxis
            ...
            tickFormatter={handlerTicketFormatter}
          />
          <YAxis
            tickMargin={10}
            dataKey={"value"}
...
const calculateDeprecationData = (
  purchaseValue,
  depreciation_frequency,
  deprecationPercent,
  sellDate,
  ...
  const usefulLifePeriods = Math.round(
    usefulLifeoftheAssets / daysOfDeprecation
  );

  const depreciationPerPeriods = Math.round(
    (initalVal - sellVal) / usefulLifePeriods
  );
  ...
  for (let i = 0; i <= usefulLifePeriods; i++) {
    ...
    depreciationData.push({
      data: shortDate(formattedDate),
      value: shortNumbers(currentValue),
    });
