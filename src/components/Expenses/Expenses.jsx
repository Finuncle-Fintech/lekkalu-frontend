...
const Expenses = () => {
  ...
  const { expenses } = useGetExpenses(
    helperUtils.createFilterUrlString({ page, per_page: rowsPerPage }),
  );

  const { handleDelete } = useDeleteExpenseRequest();
  const { handleChangeRequest } = useChangeExpenseRequest();
  const { tag: tags } = useGetTag();
  const { handleRequest } = useCreateExpense();
      ...
      if (parsedData.length > 0) {
        ...
        const loadExcel = () => {
          ...
          const promise = parsedData.map(async (entry) => {
            ...
            const createStatus = await handleRequest({
              ...entry,
  ...
  const createExpense = (expense) => {
    handleRequest({ ...expense });
  };

  const updateExpense = (expense) => {
    handleChangeRequest(expense);
  };
