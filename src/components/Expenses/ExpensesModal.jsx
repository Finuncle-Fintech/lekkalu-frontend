...
const ExpenseFormModal = ({
  ...
  const loadTags = (arr) => {
    const promises = myTags.map(async (newTag) => {
      ...
      if (!exist) {
        const newTagNameUpperCase = newTag.name.replace(
          newTag.name[0],
          newTag.name[0].toUpperCase(),
        );
  ...
  const handleSubmit = async (e) => {
    ...
    if (editIndex !== null) {
      ...
      const exist = await checkExpensesDoesNotRepeat(
        newExpense,
        axiosPrivate,
        authToken,
      );
    ...
    Swal.fire({
      ...
      title: `${
        editIndex !== null
          ? 'The expense was updated correctly'
          : 'The expense was added correctly'
      }`,
      timer: 2300,
  ...
  return (
    <>
      {!open && loadExcelStatus ? (
        <ModalExcelClosed createExpenseExcelStatus={createExpenseExcelStatus} />
      ) : (
        <></>
      )}
      <ModalContainer>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <DialogActions>
                <ReactFileReader
                  fileTypes={['.xls', '.xlsx']}
                  handleFiles={
                    !loadExcelStatus && !loadExcelStatus && handleFileUpload
                  }
