...
const Comparison = ({ id, access, name, style, className, scenarios }: ComparisonType) => {
  ...
  const handleNavigationToDetail = () => {
    return !allowRename ? navigate(`/comparisons/${id}`) : undefined
  }
  ...
  return (
    <div
      ...
      style={{ ...style }}
      onClick={handleNavigationToDetail}
    >
      <Options id={id} access={access} handleAllowRename={handleAllowRename} handleAccessChange={handleAccessChange} />
      <div className='h-full'>
        <div className='flex flex-col gap-y-3 h-full'>
          <div className={`self-center p-5 text-primary rounded-full bg-${access === 'Private' ? 'red' : 'blue'}-200`}>
